...
int t = 20;
int t2 = 25;
//перевод из 10-ой в 8-ую систему счислени€
double translation_file(double  digit, int number_system, string file_name, int accuracy) {
	...
	int str = 0;
	long long answer = 0;
	int whole_part = int(digit); // цела€ часть
	double fraction = digit - whole_part; // дробна€ часть
	int n;
...
fraction_1=fraction_1/10;
	...
	while (whole_part > 0)
	{
	   ...
	   if (d > (number_system-1) )  {
		ShowMessage( "ќшибка.\nЌажмите \"OK\" и введите корректное выражение заново.");
		return 0; }
   ...
   // перевод из дес€тичной в 16 , подаетс€ число, возврать строка
