 ...
 TB_field_bin->Text= 0;
	...
	case 18:
	  ...
	  d ="";
		...
		LongArithmetic 	num_hex, num_dec, num_oct,num_bin,d_answer;
		d_answer.StrToVector(parser(ans_s.c_str(),number_system , file_name.c_str(), StrToInt(t)).c_str());


		num_hex = d_answer.translation(number_system, 16,  file_name);
		num_dec =d_answer.translation(number_system, 10,  file_name);
		num_oct=d_answer.translation(number_system, 8,  file_name);
		num_bin=d_answer.translation(number_system, 2,  file_name);

		LongArithmetic  field =  d_answer.translation(10, number_system,  file_name);

		TB_field_dec->Text= num_dec.VectorToStr().c_str();
		TB_field_oct->Text= num_oct.VectorToStr().c_str();
		TB_field_bin->Text= num_bin.VectorToStr().c_str();
		TB_field_hex->Text= num_hex.VectorToStr().c_str();

		s = field.VectorToStr().c_str();
	...
	case 27:
		   if (s.Length()==1&& (isdigit(s[s.Length()])||s[s.Length()-1]=='e'||s[s.Length()-1]=='п'  )) {
			  ShowMessage( "Ошибка ввода.\nНажмите \"OK\", исправьте ошибку и продолжите ввод.");
		   }
		  else if (s.Length()>1) {
			if (s[s.Length()-1]==')' ||isdigit(s[s.Length()])||s[s.Length()-1]=='e'||s[s.Length()-1]=='п'  ) {
				ShowMessage( "Ошибка ввода.\nНажмите \"OK\", исправьте ошибку и продолжите ввод.");
			}else {s+=" e ";d+=" e ";}
		}else {s+=" e ";d+=" e ";}
...
if (Form1->CheckBox1->Checked == true) {
	ShowMessage( "Перейдите из режима \"длинной арифметики\" в обычный.\nНажмите \"OK\", и измените режим работы калькулятора.");
}   else {
if (number_system!=10) {
		ShowMessage( "Введите выражение в 10-ой системе счисления.\nНажмите \"OK\", и переключитесь на ввод в 10-ой системе.");
}   else{
